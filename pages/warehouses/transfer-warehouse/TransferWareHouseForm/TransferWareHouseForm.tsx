...
const TransferWareHouseForm: React.FC<TransferWareHouseFormProps> = ({
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (!detail) {
      ...
      window.location.href = `/warehouse/transfer-commands/update/${result.id}`;
      setLoading(false);
    } else {
  ...
  const getListItemSku = async () => {
    ...
    const listItemSkus =
      isArray(data.data) &&
      data.data.filter((item) => !itemIds?.includes(item.id));
    const formatListItemSkus = listItemSkus.map((item) => {
